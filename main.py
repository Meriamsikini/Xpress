...
def rag_answer(user_question, k=5):
    ...
    top_docs = list(embeddings_collection.aggregate([
        {
            "$search": {
                "index": "news_index",
                "knnBeta": {
    ...
    prompt = f"""
Tu es un assistant d'actualités. Utilise uniquement le contexte ci-dessous pour répondre à la question de l'utilisateur.
Si l'information n'est pas dans le contexte, dis "Je n'ai pas cette information dans mes sources."
...
def register(user: User):
    ...
    users_collection.insert_one({"email": user.email, "password": hashed_pw, "history": []})
...
def get_history(email: str = Query(...)):
    """
    Retourne la liste des conversations stockées dans la collection 'historiques'
    pour l'email donné. Chaque élément contient un _id (string) utilisable pour la suppression.
    """
    ...
    for d in docs:
        history.append({
            ...
            "created_at": d.get("created_at") if "created_at" in d else None
        })
...
def delete_history(email: str = Query(...), id: str = Query(...)):
    ...
    # retirer l'entrée correspondante dans users_collection.history (s'il existe)
    try:
        users_collection.update_one(
            {"email": email},
            {"$pull": {"history": {"question": doc.get("question"), "answer": doc.get("answer")}}}
        )
    except Exception:
...
def ask_question(payload: Question, email: str = Query(...)):
    ...
    for key, response in smalltalk_responses.items():
        ...
        answer = rag_answer(payload.query) # remplacer par rag_answer(payload.query)
    
    # 🔹 Sauvegarde historique
    if email:
        users_collection.update_one(
            {"email": email},
            {"$push": {"history": {"question": payload.query, "answer": answer}}}
        )
...
# 🔹 Test rapide si tu exécutes main.py directement

# 🔹 Test rapide (si on exécute main.py directement)
"""""""""
if __name__ == "__main__":
    question = "Quels sont les derniers risques liés à l'IA selon The Verge ?"
